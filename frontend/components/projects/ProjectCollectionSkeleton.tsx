    ...
    return (
        <div role="status">
            ...
            <div
                aria-hidden="true"
                className={
                    compact ? "flex flex-col gap-3" : "grid grid-cols-1 gap-9 md:grid-cols-2"
                }
            >
                {[0, 1, 2, 3].map((item) => (
                    <div
                        key={item}
                        className={`border border-border bg-card ${compact ? "rounded-xl p-4" : "min-h-72 rounded-[24px] p-6 sm:p-8"}`}
                    >
