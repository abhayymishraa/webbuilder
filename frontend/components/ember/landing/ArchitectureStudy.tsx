    ...
    >
      <div className="studies-archCopy relative z-1 min-w-0 [&_h1_em]:[display:inline-block] [&_h1_em]:pb-[5px] [&_h1_em]:text-[var(--study-accent)] max-md:p-0">
        ...
        {hero ? (
          ...
          <div
            className="studies-layerControls flex flex-col max-w-82.5 gap-0 mt-4.5 mx-0 mb-3 [&_button]:flex [&_button]:items-center [&_button]:gap-4 [&_button]:min-h-11 [&_button]:border-0 [&_button]:bg-transparent [&_button]:text-[var(--study-muted)] [&_button]:text-left [&_button]:py-3.5 [&_button]:px-0 [&_button]:text-[14px] [&_button_span:first-child]:[font:10px_monospace] [&_button_span:first-child]:opacity-70 [&_button_span:last-child]:ml-auto [&_button[aria-pressed=true]]:text-[var(--study-accent)] [&_button:hover]:text-[var(--study-text)] max-md:mt-4 max-md:mx-0 max-md:mb-2.5 max-md:max-w-full max-md:flex-row max-md:gap-3.5 max-md:flex-wrap max-md:[&_button]:text-[12px] max-md:[&_button]:gap-2 max-md:[&_button]:py-3 max-md:[&_button]:px-0 max-md:[&_button_span:last-child]:hidden"
            role="group"
            aria-label="Inspect architecture layers"
          >
            {layers.map((layer, index) => (
              <button
                key={layer.name}
